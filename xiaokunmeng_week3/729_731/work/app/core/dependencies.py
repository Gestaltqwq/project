...
def _authenticate() -> User:
    ...
    # 校验黑名单：登出过的 Token 直接拒绝（防 JWT 登出漏洞）
    from app.models.token_blacklist import TokenBlacklist
    db = get_db()
    if TokenBlacklist.is_blacklisted(db, token):
        raise BizException(1002, "Token已失效，请重新登录", 401)
    payload = decode_payload(token)
    if not payload or not payload.get("sub"):
        raise BizException(1002, "Token无效或已过期", 401)
    # JWT subject = 用户 ID（改名不影响登录态）
    try:
        user = User.find_by_id(db, int(payload["sub"]))
    except (ValueError, TypeError):
        raise BizException(1002, "Token无效或已过期", 401)
    if not user:
        raise BizException(1002, "用户不存在", 401)
    # 令牌版本校验：改密后旧 token 失效（增强）
    ver = payload.get("ver")
    if ver is not None and ver != user.token_version:
        raise BizException(1002, "登录已失效，请重新登录", 401)
    g.current_user = user
    return user
